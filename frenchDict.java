...
class frenchDict{
  ...
  public void promptSearch(String key){
    ...
    //if I explicitly prompted this method with `lookup`, tell tell me that you could not find the word
    else if (keep.isEmpty()) System.out.println(OPEN_BOLD + "I could not find " + "\"" + key + "\" in your dictionary." + CLOSE_BOLD);
  }
  ...
  public Map<String,Stack<String>> mismatch(){
    ...
    */
    ...
    // add all keys to a vector so  we can randomly index into the vector, removing values as we go!
    ...
    if (answer.equals("y") || answer.contains("yes")){
      ...
      for (int i = 0; i < ranNum ; i++){
        ...
        for (Map.Entry<String,Stack<String>> each : mismatched.entrySet()){
          ...
          String wackyString = reorderChar(actualKey);
          //rearrange each definition in value
          Stack<String> definitions  = each.getValue();
          for (int j = 0; j < definitions.size(); j++){
            definitions.set(j, reorderChar(definitions.get(j)));
          }
          wacky.put(wackyString,definitions);
        }
...
"10. help (display this help message screen)");
  ...
  private String reorderChar(String str){
    ...
    while (!list.isEmpty()){
      ...
      ranNum = rand.nextInt(list.size());
      //get that randomly selected character out of the vector
      wackyString += list.get(ranNum);
